...
async function handleFileUpload(file, type) {
    ...
    // Access module instance from global scope
    if (!window.segmentationModule) {
        console.error('[fileUpload] Segmentation module not initialized');
        return;
    }

    window.segmentationModule.uploadedFiles[type] = file;

    if (type === 'rawImages' || type === 'annotations') {
        updateUploadStatus(type, file.name);

        // If both files are uploaded, validate them
        if (window.segmentationModule.uploadedFiles.rawImages && window.segmentationModule.uploadedFiles.annotations) {
...
async function validateTiffStacks() {
    ...
    formData.append('raw_images', window.segmentationModule.uploadedFiles.rawImages);
    formData.append('annotations', window.segmentationModule.uploadedFiles.annotations);
