...
class BaseTrainer:
    ...
    def train_epoch(self, train_loader):
        ...
        Handles both 2D and 2.5D modes:
        - 2D: masks come as (B, H, W), need unsqueeze to (B, 1, H, W)
        - 2.5D: masks come as (B, 3, H, W), already have channel dimension
        ...
        for inputs, targets, masks in train_loader:
            ...
            # Handle mask dimensions for both 2D and 2.5D modes
            # 2D mode: masks are (B, H, W) -> need (B, 1, H, W)
            # 2.5D mode: masks are already (B, 3, H, W)
            if masks.dim() == 3:
    ...
    def calculate_loss(self, pred, target, mask):
        """
        Calculate the masked loss for model training.

        Works for both 2D and 2.5D modes:
        - 2D: pred/target/mask shapes are (B, 1, H, W)
        - 2.5D: pred/target/mask shapes are (B, C, H, W) where C=1 or C=3

        The loss is computed only over masked pixels using sum/count formula
        to ensure proper weighting regardless of mask density.

        Args:
            pred (torch.Tensor): Predicted tensor of shape (B, C, H, W)
            target (torch.Tensor): Target tensor of shape (B, C, H, W)
            mask (torch.Tensor): Boolean/float mask tensor of shape (B, C, H, W)
                indicating which pixels to include in loss calculation

        Returns:
            torch.Tensor: Calculated loss value (scalar)
        """
        # Validate input shapes
        if pred.shape != target.shape:
            raise ValueError(f"Shape mismatch: pred {pred.shape} vs target {target.shape}")

        # Ensure mask has same shape as pred/target
        ...
        # Apply mask and compute loss as sum / count (proper masked loss)
        # This ensures correct weighting regardless of mask density
