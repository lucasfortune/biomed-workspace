...
class StructuralNoiseExtractor:
    ...
    def _calculate_3d_autocorrelation(self, triplet):
        """
        Calculate 3D autocorrelation for a 3-slice triplet.

        Args:
            triplet (numpy.ndarray): 3D array of shape (3, H, W)

        Returns:
            numpy.ndarray: 3D autocorrelation of shape (5, 2H-1, 2W-1),
                          but we only keep dz in {-1, 0, +1} -> (3, 2H-1, 2W-1)
        """
        # Convert to numpy if tensor
        if hasattr(triplet, 'numpy'):
            triplet = triplet.numpy()

        # Ensure correct shape
        if triplet.ndim != 3 or triplet.shape[0] != 3:
            raise ValueError(f"Expected triplet of shape (3, H, W), got {triplet.shape}")

        # Center the noise pattern (per-slice mean subtraction)
        triplet_centered = triplet - triplet.mean(axis=(1, 2), keepdims=True)

        # Calculate 3D autocorrelation using FFT convolution
        # Full 3D autocorrelation: convolve with flipped version
        flipped = np.flip(triplet_centered)  # Flip along all axes
        autocorr_3d_full = signal.fftconvolve(triplet_centered, flipped, mode='full')
        # autocorr_3d_full shape: (5, 2H-1, 2W-1) for 3-slice input

        # Extract only dz in {-1, 0, +1} from the center
        # For input shape (3,), autocorr shape along z is (5,): indices 0,1,2,3,4 map to dz=-2,-1,0,+1,+2
        # We want indices 1,2,3 which correspond to dz=-1,0,+1
        autocorr_3d = autocorr_3d_full[1:4, :, :]  # Shape: (3, 2H-1, 2W-1)

        if self.norm_autocorr:
            # Normalize by center value (dz=0, center spatial position)
            center_z = 1  # dz=0 is at index 1
            center_y, center_x = autocorr_3d.shape[1] // 2, autocorr_3d.shape[2] // 2
            center_val = autocorr_3d[center_z, center_y, center_x]
            autocorr_3d = autocorr_3d / (center_val + 1e-10)

        if self.log_autocorr:
            autocorr_3d = np.log1p(np.abs(autocorr_3d))

        return autocorr_3d

    def extract_mask_3d(self, noise_patterns_3d, verbose=False, edge_slice_params=None):
        """
        Extract 3D mask from triplet patches for 2.5D mode.

        This method computes 3D autocorrelation for each triplet, averages them,
        then applies 2D mask extraction to each dz slice with appropriate parameters:
        - dz=0 (center): uses base extractor params
        - dz=-1, dz=+1 (edges): uses derived edge params (more aggressive thresholding)

        Args:
            noise_patterns_3d (numpy.ndarray): Array of triplet patches, shape (N, 3, H, W)
            verbose (bool): Whether to print debug info and show visualizations
            edge_slice_params (dict, optional): Override params for edge slices (dz!=0).
                If None, will auto-derive from current extractor params.

        Returns:
            tuple: (binary_mask_3d, center_squares_3d)
                - binary_mask_3d: Shape (3, kernel_h, kernel_w) - 3D structural kernel
                - center_squares_3d: Shape (3, kernel_h, kernel_w) - processed autocorrelations
        """
        # Reset cache
        self.clear_cache()

        # Convert tensor to numpy if needed
        if hasattr(noise_patterns_3d, 'numpy'):
            noise_patterns_3d = noise_patterns_3d.numpy()

        # Validate input shape
        if noise_patterns_3d.ndim != 4 or noise_patterns_3d.shape[1] != 3:
            raise ValueError(f"Expected shape (N, 3, H, W), got {noise_patterns_3d.shape}")

        # Compute 3D autocorrelation for each triplet and average
        autocorrs_3d = []
        for i in range(noise_patterns_3d.shape[0]):
            triplet = noise_patterns_3d[i]  # (3, H, W)
            autocorr = self._calculate_3d_autocorrelation(triplet)
            autocorrs_3d.append(autocorr)

        # Average all autocorrelations
        avg_autocorr_3d = np.mean(autocorrs_3d, axis=0)  # Shape: (3, 2H-1, 2W-1)

        if verbose:
            print(f"Averaged 3D autocorrelations from {len(autocorrs_3d)} triplets")
            print(f"Averaged autocorr shape: {avg_autocorr_3d.shape}")

        # Derive edge slice params if not provided
        if edge_slice_params is None:
            # Lazy import to avoid circular dependency
            from ..pipeline.config import derive_edge_slice_params
            base_params = {
                'adapt_CB': self.adapt_CB,
                'adapt_DF': self.adapt_DF,
                'base_percentile': self.base_percentile,
                'percentile_decay': self.percentile_decay,
                'max_true_pixels': self.max_true_pixels
            }
            edge_slice_params = derive_edge_slice_params(base_params)

        # Extract 2D mask for each dz slice
        binary_masks = []
        center_squares = []

        # Process each dz slice: indices 0,1,2 correspond to dz=-1,0,+1
        dz_labels = {0: 'dz=-1 (edge)', 1: 'dz=0 (center)', 2: 'dz=+1 (edge)'}

        for dz_idx in range(3):
            autocorr_2d = avg_autocorr_3d[dz_idx]  # (2H-1, 2W-1)

            if verbose:
                print(f"\n--- Processing {dz_labels[dz_idx]} ---")

            # Use base params for center slice, edge params for edge slices
            if dz_idx == 1:  # Center slice (dz=0)
                # Use base extractor parameters (already set)
                pass
            else:  # Edge slices (dz=-1 or dz=+1)
                # Temporarily override params with edge params
                orig_adapt_CB = self.adapt_CB
                orig_adapt_DF = self.adapt_DF
                orig_base_percentile = self.base_percentile
                orig_percentile_decay = self.percentile_decay
                orig_max_true_pixels = self.max_true_pixels

                self.adapt_CB = edge_slice_params['adapt_CB']
                self.adapt_DF = edge_slice_params['adapt_DF']
                self.base_percentile = edge_slice_params['base_percentile']
                self.percentile_decay = edge_slice_params['percentile_decay']
                self.max_true_pixels = edge_slice_params['max_true_pixels']

            # Process this 2D slice
            # Set up autocorr for 2D extraction
            self._autocorr = autocorr_2d
            self._center_square = self._crop_and_adapt(autocorr_2d)
            self._original_autocorr_values = self._center_square.copy()
            ...
            # Pre-compute ring coordinates
            max_rings = max(center_y, center_x) + 1
            self._ring_coordinates = self._create_ring_coordinates(center_y, center_x, max_rings)

            # Process each ring
            for ring_idx, ring_coords in enumerate(self._ring_coordinates, start=1):
                threshold, should_process, above_threshold_mask = self._process_ring(
                    ring_coords, ring_idx, verbose
                )
            ...
            # Keep only connected component with center
            if self.keep_center_component_only:
                ...
                if center_label > 0:
                    ...
                    num_components = np.max(labeled_mask)
                    if num_components > 1 and verbose:
                        print(f"Removed {num_components - 1} disconnected components")

            # Apply size limit
            binary_mask = self._limit_mask_size(binary_mask)

            binary_masks.append(binary_mask)
            center_squares.append(self._center_square.copy())

            # Restore original params if we changed them
            if dz_idx != 1:
                self.adapt_CB = orig_adapt_CB
                self.adapt_DF = orig_adapt_DF
                self.base_percentile = orig_base_percentile
                self.percentile_decay = orig_percentile_decay
                self.max_true_pixels = orig_max_true_pixels

        # Stack into 3D arrays
        binary_mask_3d = np.stack(binary_masks, axis=0)  # (3, kernel_h, kernel_w)
        center_squares_3d = np.stack(center_squares, axis=0)  # (3, kernel_h, kernel_w)

        if verbose:
            import matplotlib.pyplot as plt

            print("\n=== 3D Structural Noise Extraction ===")
            print(f"3D Mask shape: {binary_mask_3d.shape}")
            for dz_idx in range(3):
                print(f"  {dz_labels[dz_idx]}: {np.sum(binary_mask_3d[dz_idx])} True pixels")

            # Visualize all 3 slices
            fig, axes = plt.subplots(2, 3, figsize=(15, 8))

            for dz_idx in range(3):
                axes[0, dz_idx].imshow(center_squares_3d[dz_idx], cmap='viridis')
                axes[0, dz_idx].set_title(f"Autocorr {dz_labels[dz_idx]}")
                axes[1, dz_idx].imshow(binary_mask_3d[dz_idx], cmap='gray')
                axes[1, dz_idx].set_title(f"Mask {dz_labels[dz_idx]}")

            plt.tight_layout()
            plt.show()

        return binary_mask_3d, center_squares_3d
