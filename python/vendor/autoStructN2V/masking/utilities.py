...
def create_full_mask(single_masking_kernel, patch_size, mask_percentage, verbose=False):
    ...
    # Get dimensions and properties of the input pattern. Kernels may be
    # non-square (e.g. a 9x17 horizontal mask for vertical stripe noise), so
    # track height (ph) and width (pw) independently rather than assuming a
    # single square pattern_size.
    ph, pw = single_masking_kernel.shape
    ...
    # Create a grid of all possible top-left positions. Row positions span
    # patch_size - ph + 1 and column positions span patch_size - pw + 1, so a
    # wider-than-tall (or taller-than-wide) kernel still fits. indexing='ij'
    # makes the first column of `positions` the row index (i) and the second
    # the column index (j), matching how the kernel is sliced below.
    y_grid, x_grid = np.meshgrid(
        np.arange(patch_size - ph + 1),
        np.arange(patch_size - pw + 1),
        indexing='ij',
    )
    positions = np.column_stack((y_grid.ravel(), x_grid.ravel()))
    ...
    for (i, j) in positions:
        ...
        forbidden_local = forbidden[i:i + ph, j:j + pw]
        if np.any(single_masking_kernel & forbidden_local):
            continue

        # Place the pattern
        full_masking_kernel[i:i + ph, j:j + pw] |= single_masking_kernel
        cy = i + center_y
        cx = j + center_x
        ...
        x0 = max(0, j - 1)
        x1 = min(patch_size, j + pw + 1)
        h_loc = y1 - y0
        w_loc = x1 - x0
        newly_placed = np.zeros((h_loc, w_loc), dtype=bool)
        ny = i - y0
        nx = j - x0
        newly_placed[ny:ny + ph, nx:nx + pw] = single_masking_kernel
