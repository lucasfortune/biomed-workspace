...
class _NullSummaryWriter:
    ...
    Per-epoch event files were the dominant disk consumer (~450 MB/variant,
    ~109 GB across all runs as of 2026-06-26) yet are not consumed by the
    experiment pipeline (results come from ``metrics.json``). Disabling logging
    by default keeps reruns from re-bloating the volume. Any ``add_*`` / ``close``
    / ``flush`` call becomes a no-op; set ``hparams['enable_tensorboard']=True``
    to restore real logging.
    """
...
class AutoStructN2VTrainer(BaseTrainer):
    ...
    def __init__(self, model, optimizer, scheduler, device, hparams, stage,
        ...
        # Legacy stage names ('stage1'/'stage2') and routed branch names
        # ('n2v'/'structn2v', restructure 2026-08-25) are both accepted. The
        # routed names change nothing about training; they only pick the recipe
        # lookup key and the log-dir name, and skip the legacy stage-1
        # denoised-patch export (which only the retired two-stage flow consumed).
        self.stage = stage
        if stage not in ('stage1', 'stage2', 'n2v', 'structn2v'):
            raise ValueError(
                f"Invalid stage: {stage}. Must be 'stage1', 'stage2', 'n2v' or 'structn2v'.")
        self._is_n2v_like = stage in ('stage1', 'n2v')
        ...
        stage_name = 'n2v' if self._is_n2v_like else 'structn2v'
        ...
        # TensorBoard logging is OFF by default — the event files were the
        # dominant disk consumer (~450 MB/variant) and are not used by the
        # results pipeline. Opt back in via hparams['enable_tensorboard']=True.
        # Note: self.log_dir is still used below to save best_model.pth, so it
        # is created regardless of this flag.
        self.enable_tensorboard = bool(hparams.get('enable_tensorboard', False))
        self.writer = (SummaryWriter(self.log_dir) if self.enable_tensorboard
    ...
    def _get_param(self, param_name, default_value=None):
        ...
        Returns:
            Parameter value
        """
        # Structured lookup: hparams[<stage key>] — works for legacy ('stage1'/
        # 'stage2') and routed ('n2v'/'structn2v'; the runner stores the branch
        # recipe under the branch name).
        block = self.hparams.get(self.stage)
        if isinstance(block, dict) and param_name in block:
            return block[param_name]
        # Legacy flat-format fallback (n2v_*/structn2v_* prefixes).
        prefix = 'n2v' if self._is_n2v_like else 'structn2v'
        return self.hparams.get(f'{prefix}_{param_name}', default_value)
    ...
    def train(self, train_loader, val_loader, test_loader=None):
        ...
        Returns:
            ...
            For stage2: None
        """
        # Validate inputs based on stage (routed stages treat test_loader as optional)
        ...
        #
        # PERF FIX (2026-06-17 session 03): the aux PSNR is now computed ONLY
        # when it will actually drive scheduling / early-stopping / checkpoint
        # selection (``use_aux_psnr_for_scheduling=True``). Previously it was
        # computed and logged EVERY epoch whenever a clean reference was present,
        # even though the paper-faithful default (flag False) never consumes it —
        # ~105 full 250×250 tiled forward passes per epoch that nothing reads,
        # roughly DOUBLING per-epoch wall time on COSEM. That logged-but-unused
        # path is removed from the training loop here. The default (flag False)
        # now does ZERO aux-PSNR inferences and drives all decisions from the
        # masked val_loss (Krull 2019, Broaddus 2020, Höck 2022, CAREamics
        # 0.1.0). The opt-in scheduling ablation (flag True) is unchanged.
        flag_use_aux = bool(self.hparams.get('use_aux_psnr_for_scheduling', False))
        compute_aux_psnr = (flag_use_aux
                            and self.clean_stack is not None
        ...
        if compute_aux_psnr:
            ...
            if getattr(train_ds, 'stack', None) is not None:
                ...
                print("Warning: use_aux_psnr_for_scheduling=True but training dataset "
                      "is in path mode (no indexable noisy stack). Falling back to "
                      "masked val_loss for all decisions.")
                compute_aux_psnr = False
                self._noisy_stack_ref = None
        else:
            self._noisy_stack_ref = None

        drive_scheduling_with_aux_psnr = compute_aux_psnr
        ...
        with tqdm(total=num_epochs, desc="Training Progress", ncols=100) as pbar:
            for epoch in range(num_epochs):
                ...
                # Auxiliary PSNR-against-clean — computed ONLY when it drives
                # scheduling (use_aux_psnr_for_scheduling=True). The paper-
                # faithful default skips it entirely (perf fix, session 03) and
                # uses the masked val_loss below.
                if compute_aux_psnr:
        ...
        # Routed branches ('n2v'/'structn2v'): optional test pass, no legacy
        # side-products (the denoised-patch export only fed the retired
        # two-stage mask path).
        if self.stage in ('n2v', 'structn2v'):
            if test_loader is not None:
                print("Starting testing phase...")
                patch_size = self._get_param('patch_size', 64)
                test_loss = self.test_model(test_loader, self.writer, patch_size,
                                            patch_size // 2)
                print(f"Testing completed. Test Loss: {test_loss:.4f}")
            self.writer.close()
            return None

        # Legacy stage-specific post-training actions
