...
class FlexibleUNet(nn.Module):
    ...
    - Each conv block: Conv2D → Norm → activation → Conv2D → Norm → activation,
      with reflection padding around each conv. The normalization layer is
      selectable via ``norm_type`` (BatchNorm by default).
    ...
    Args:
        ...
        norm_type (str): Normalization layer in conv blocks: 'batch'
            (``nn.BatchNorm2d``, legacy default), 'group' (``nn.GroupNorm``),
            'instance' (``nn.GroupNorm`` with one group per channel, i.e.
            InstanceNorm with affine params), or 'none' (no normalization).
            BatchNorm keeps running statistics that are accumulated in train
            mode and frozen at inference; under heavy structured-mask input
            corruption (StructN2V, ~27% of pixels) those running stats become
            miscalibrated relative to clean inference inputs, collapsing
            output quality (root-caused 2026-06-17). 'group'/'instance'/'none'
            normalize per-sample with no train/inference running-stat split
            and avoid that failure mode. Defaults to 'batch' for backward
            compatibility.
        num_groups (int): Number of groups for ``norm_type='group'``. Must
            divide every conv-block channel count. Defaults to 8.
    ...
    def __init__(self, features, num_layers, in_channels=1, out_channels=1,
        ...
        if norm_type not in ('batch', 'group', 'instance', 'none'):
            raise ValueError(
                f"norm_type must be 'batch', 'group', 'instance', or 'none', got {norm_type!r}")
        if norm_type == 'group' and num_groups <= 0:
            raise ValueError(f"num_groups must be a positive integer, got {num_groups}")
        if init_scale <= 0:
            raise ValueError(f"init_scale must be a positive float, got {init_scale}")
        ...
        # Multiplicative factor applied to every conv weight AFTER kaiming init.
        # Default 1.0 = the textbook He init (var 2/fan_in). The audit
        # (2026-06-23) traced our 10^3-10^5x conv ill-conditioning vs CAREamics
        # to this init being sqrt(6)x too large under a norm-after-every-conv
        # design; init_scale=1/sqrt(6) rescales var to 1/(3*fan_in) (CAREamics'
        # PyTorch-default range). See COMPARISON_vs_careamics.md.
    ...
    def _make_norm(self, channels):
        """Build the normalization layer selected by ``self.norm_type``.

        'batch'    → ``nn.BatchNorm2d`` (keeps running stats; legacy default).
        'group'    → ``nn.GroupNorm(num_groups, channels)``.
        'instance' → ``nn.GroupNorm(channels, channels)`` (one group per
                     channel = InstanceNorm with affine params, no running
                     stats — unlike ``nn.InstanceNorm2d`` whose affine default
                     is off).
        'none'     → ``nn.Identity``.
        """
        ...
        if self.norm_type == 'group':
            if channels % self.num_groups != 0:
                raise ValueError(
                    f"num_groups={self.num_groups} does not divide channel count "
                    f"{channels}; choose a num_groups that divides every conv-block "
                    f"channel width.")
    ...
    def conv_block(self, in_channels, out_channels, name=None):
        """Double-conv block with reflection padding and normalization.

        The normalization layer is selected by ``self.norm_type`` and the
        activation by ``self.activation`` ('elu' or 'relu').
        """
    ...
    def _init_weights(self, m):
        ...
        if isinstance(m, (nn.Conv2d, nn.ConvTranspose2d)):
            ...
            if self._init_scale != 1.0:
                with torch.no_grad():
                    m.weight.mul_(self._init_scale)
