...
function createWorkspaceRoutes(dependencies) {
  ...
  /**
   * Copy a built-in test stack into the workspace and register it exactly like
   * an upload, so every module's FileSelector can offer test data through the
   * same code path as a real file (consolidation decision 5: raw image inputs
   * always offer the test stack, mask inputs where a test mask exists).
   * POST /api/workspace/test-data  body: { kind: 'raw' | 'inference' | 'denoising' | 'annotations' }
   * Idempotent: a stack that is already in the workspace is returned as is.
   */
  const TEST_STACKS = {
    raw:         { file: 'trypB_testData_training.tif',    subdir: 'uploads/raw',         tags: ['raw', 'test-data'] },
    inference:   { file: 'trypB_testData_inference.tif',   subdir: 'uploads/raw',         tags: ['raw', 'test-data'] },
    denoising:   { file: 'trypB_testData_denoising.tif',   subdir: 'uploads/raw',         tags: ['raw', 'test-data'] },
    annotations: { file: 'trypB_testData_annotations.tif', subdir: 'uploads/annotations', tags: ['annotation', 'test-data'] }
  };

  router.post('/test-data', requireAuth, async (req, res) => {
    try {
      const sessionId = req.session.id;
      const kind = String(req.body?.kind || 'raw');
      const spec = TEST_STACKS[kind];
      if (!spec) {
        return res.status(400).json({
          success: false,
          error: `Unknown test data kind "${kind}"`,
          details: `Expected one of: ${Object.keys(TEST_STACKS).join(', ')}`
        });
      }

      const sourcePath = path.join('test_data', spec.file);
      if (!fs.existsSync(sourcePath)) {
        return res.status(404).json({
          success: false,
          error: 'Test data not found on the server',
          details: `Expected file: ${sourcePath}`
        });
      }

      const workspacePath = workspaceService.getWorkspacePath(sessionId);
      if (!fs.existsSync(workspacePath)) {
        workspaceService.initializeWorkspace(sessionId);
      }
      ...
      if (fileService && !fileEntry.thumbnailPath) {
        fileService.generateThumbnailAsync(sessionId, destPath, fileEntry.id);
      }
      ...
      if (logger) logger.error('[Workspace] Error loading test data:', error);
